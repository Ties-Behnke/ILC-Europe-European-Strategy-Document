...
\begin{figure}
\begin{center}
%\includegraphics[width=0.80\hsize]{WellsZhang.pdf}
\end{center}
...
\begin{figure}
\begin{center}
%\includegraphics[width=0.80\hsize]{HiggsEvents.pdf}
\end{center}
...
\begin{figure}
\begin{center}
%\includegraphics[width=0.80\hsize]{eeZhdiagrams.pdf}
\end{center}
...
After we describe the experimental methods and the expected measurement uncertainties in Section~xx, we will present the projections for uncertainties in 
...
\begin{figure}
\begin{center}
%\includegraphics[width=0.80\hsize]{eeWWdiagrams.pdf}
\end{center}
...
In Section xx.  below, after describing the experimental study of $\ee\to W^+W^-$, we will show that the ILC at 250~GeV is expected to improve the precision of $W$ form factor measurements by an order of magnitude over expected results from HL-LHC.
...
The reaction $\ee\to b\bar b$ deserves special consideration. In models in which the Higgs boson is composite, typically either the $t_L$ or the $t_R$ must be composite also to generate a large enough $t$ quark mass.    The $b_L$ is the $SU(2)$ partner of the $t_L$ and so must have the same admixture of composite structure.    If it is the $t_R$ that is more composite, it is not required that the $b_R$ is composite, but this often happens in models.  This can generate few-percent corrections to the rates for $e^-_{L,R} e^+\to b_R\bar b_L$ at the ILC.  ~\cite{Funatsu:2017nfm,YoonPeskin}.   It is possibe that this effect, rather than effects in Higgs decays, would be the first indication of a composite Higgs sector. 
...
The experimental aspects of these particle seaches are discussed in Section~xx.  A broader review of the opportunities for new particle discovery at $\ee$ colliders can be found in \cite{Fujii:2017ekh}.
